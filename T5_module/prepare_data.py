...
def main(args = None):
    ...
    cache = user_cache_dir('huggingface') #If there is a previous training set cached, the model will use it instead of the new, thus we clear it
    ...
    if data_args.train_file_name is None:
        ...
        train_path = os.path.join(os.path.dirname(__file__)+"/data", train_file_name)

        valid_file_name = f"valid_data_{data_args.task}_{data_args.qg_format}_{data_args.model_type}.pt"
        valid_path = os.path.join(os.path.dirname(__file__)+"/data", valid_file_name)
    else:
        train_path = os.path.join(os.path.dirname(__file__)+"/data", data_args.train_file_name)
        valid_path = os.path.join(os.path.dirname(__file__)+"/data", data_args.valid_file_name)
